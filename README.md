# 📘 Creditos API (.NET 6, Kafka, Docker, PostgreSQL)

API RESTful desenvolvida em **.NET 6** para integração e consulta de créditos constituídos, contendo:

- Publicação de mensagens no Kafka
- Background service consumindo e persistindo créditos individualmente (não em bulk)
- Endpoints REST completos
- Banco PostgreSQL em container
- Testes unitários com xUnit
- Docker + Docker Compose
- Implementação com boas práticas de DDD, Clean Code e SOLID

## 📦 Tecnologias Utilizadas

| Categoria       | Tecnologia                           |
|----------------|---------------------------------------|
| Linguagem      | **C# / .NET 6**                       |
| Web API        | ASP.NET Core / Controllers            |
| ORM            | **Entity Framework Core (Npgsql)**    |
| Banco          | **PostgreSQL** (containerizado)       |
| Mensageria     | **Kafka** (producer + consumer)       |
| Container      | Docker / Docker Compose               |
| Testes         | xUnit                                 |

## 🏗 Arquitetura do Projeto (DDD simplificado)

```
CreditApi/
│
├── Controllers/          → APIs REST (CreditController, HealthController)
├── Data/                 → AppDbContext, Mappings
├── Background/           → KafkaConsumerService
├── Services/             → KafkaPublisher, CreditService
├── Repositories/         → CreditoRepository (EF)
├── Models/               → Entidade Credito
├── DTOs/                 → Requests e Responses
├── Migrations/           → Arquivos SQL
└── Program.cs
```

## 🚀 Como Rodar o Projeto (Docker)

> **Pré-requisitos:** Docker + Docker Compose

1. Ajuste as variáveis de ambiente no `docker-compose.yml` se necessário.  
2. Execute:

```bash
docker compose up --build
```

3. A API estará disponível em:

```
http://localhost:5000
```

4. Acesse o Swagger:

```
http://localhost:5000/swagger
```

## 🔥 Endpoints

### **POST /api/creditos/integrar-credito-constituido**

Recebe uma lista de créditos e envia **cada item como uma mensagem distinta** ao Kafka no tópico:

```
integrar-credito-constituido-entry
```

#### ✔ Retorno esperado:
```json
{
  "success": true
}
```

### **GET /api/creditos/{numeroNfse}**

Retorna todos os créditos referentes ao mesmo número da NFS-e.

### **GET /api/creditos/credito/{numeroCredito}**

Retorna somente um crédito específico.

## 💾 Modelagem da Tabela

```sql
CREATE TABLE credito
(
    id                BIGINT GENERATED BY DEFAULT AS IDENTITY,
    numero_credito    VARCHAR(50)    NOT NULL,
    numero_nfse       VARCHAR(50)    NOT NULL,
    data_constituicao DATE           NOT NULL,
    valor_issqn       DECIMAL(15, 2) NOT NULL,
    tipo_credito      VARCHAR(50)    NOT NULL,
    simples_nacional  BOOLEAN        NOT NULL,
    aliquota          DECIMAL(5, 2)  NOT NULL,
    valor_faturado    DECIMAL(15, 2) NOT NULL,
    valor_deducao     DECIMAL(15, 2) NOT NULL,
    base_calculo      DECIMAL(15, 2) NOT NULL
);
```

## 🔄 Background Service

Um hosted service executa a cada **500 ms**:

- Lê mensagens do Kafka
- Converte para objeto de domínio
- Insere **uma por vez** no banco (não usa bulk)

## ❤️ Endpoints de Saúde

### ✔ `/self`
Indica se o serviço está ativo.

### ✔ `/ready`
Indica se o serviço está pronto (banco e Kafka funcionando).

## 🧪 Testes Automatizados

- Projeto `CreditApi.Tests`
- Baseado em **xUnit**
- Testes exemplo incluem:
  - Repositório
  - Serviço
  - Validações

# 📋 CHECKLIST DO DESAFIO

| Requisito | Status |
|----------|--------|
| .NET 6 | ✔ |
| API RESTful | ✔ |
| POST integrando créditos | ✔ |
| Geração de mensagens individuais no Kafka | ✔ |
| Consumer lendo a cada 500 ms | ✔ |
| Inserção individual (não bulk) | ✔ |
| Banco PostgreSQL em container | ✔ |
| Persistência dos dados | ✔ |
| GET por NFS-e | ✔ |
| GET por número do crédito | ✔ |
| Endpoints de saúde | ✔ |
| Dockerfile / docker-compose | ✔ |
| Código limpo e organizado | ✔ |
| Uso de padrões | ✔ |
| Testes unitários | ✔ |
| README completo | ✔ |

## 📦 Checklist antes de subir no GitHub

✔ Confirmar que todos os commits estão organizados  
✔ Conferir `.dockerignore`  
✔ Validar build com Docker  
✔ Validar consumer e publisher  
✔ Rodar testes unitários  
✔ Revisar Swagger  


## EXEMPLO DE DADOS
```
[

               {

                 "numeroCredito": "123456",

                 "numeroNfse": "7891011",

                 "dataConstituicao": "2024-02-25",

                 "valorIssqn": 1500.75,

                 "tipoCredito": "ISSQN",

                 "simplesNacional": "Sim",

                 "aliquota": 5.0,

                 "valorFaturado": 30000.00,

                 "valorDeducao": 5000.00,

                 "baseCalculo": 25000.00

               },

               {

                 "numeroCredito": "789012",

                 "numeroNfse": "7891011",

                 "dataConstituicao": "2024-02-26",

                 "valorIssqn": 1200.50,

                 "tipoCredito": "ISSQN",

                 "simplesNacional": "Não",

                 "aliquota": 4.5,

                 "valorFaturado": 25000.00,

                 "valorDeducao": 4000.00,

                 "baseCalculo": 21000.00

               },

               {

                 "numeroCredito": "654321",

                 "numeroNfse": "1122334",

                 "dataConstituicao": "2024-01-15",

                 "valorIssqn": 800.50,

                 "tipoCredito": "Outros",

                 "simplesNacional": "Sim",

                 "aliquota": 3.5,

                 "valorFaturado": 20000.00,

                 "valorDeducao": 3000.00,

                 "baseCalculo": 17000.00

               }

]
```
